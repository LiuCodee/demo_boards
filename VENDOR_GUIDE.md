...
First prepare the board materials: schematics, hardware feature descriptions,
and board initialization code from existing projects, if available.

Configure [ESP Pilot MCP](https://mcp.espressif.com/#esp-pilot-mcp) in your AI
tool, then provide those materials to the AI to help create or migrate board
configurations. If its introduction page is unavailable, give the AI assistant
this direct URL: `https://mcp.esp-pilot.espressif.com/mcp`.
...
After adding boards, run
`python scripts/update_supported_boards_table.py` to update the board tables in
both README files. The script does not modify existing table content; it only
appends the board name and chip for new boards at the end of the table.

Submit board changes through a pull request. CI generates and builds every
supported board combination across the declared Board Manager lower bound, the
latest version, ESP-IDF `v5.5.4`, and `latest`.
